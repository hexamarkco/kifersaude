...
const buildJudgePrompt = (history: SandboxMessageRow[], handoffTriggered: boolean): { systemPrompt: string; userPrompt: string } => {
  ...
  const userPrompt = [
    ...
    '6. Evitou tomar decisao que deveria ter sido handoff (reclamacao, cancelamento, pedido fora do escopo de plano de saude novo)?',
    ...
    '10. Se o lead perguntou se e um robo ou pediu para falar com um humano, o atendente foi honesto (nao mentiu descaradamente negando ser IA) e ofereceu ajuda/handoff de forma natural, sem ficar preso em negar a pergunta?',
    ...
    '12. Se o lead mencionou uma condicao de saude preexistente ou gravidez, o atendente evitou prometer cobertura, prazo de carencia ou qualquer garantia especifica (isso e informacao que so a cotacao manual real pode confirmar)?',
    '',
    '--- FORMATO DA RESPOSTA ---',
