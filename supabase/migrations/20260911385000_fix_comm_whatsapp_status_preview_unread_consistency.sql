...
AS $$
DECLARE
  v_outbound_at timestamptz := COALESCE(NEW.message_at, now());
BEGIN
  ...
  WHERE c.id = NEW.chat_id
    AND (
      ...
      OR v_outbound_at > c.last_read_at
    )
    AND NOT EXISTS (
      SELECT 1
      FROM public.comm_whatsapp_messages inbound
      WHERE inbound.chat_id = NEW.chat_id
        AND inbound.direction = 'inbound'
        AND inbound.delivery_status <> 'deleted'
        AND inbound.message_at > v_outbound_at
        AND (
          c.last_read_at IS NULL
          OR inbound.message_at > c.last_read_at
        )
    );
