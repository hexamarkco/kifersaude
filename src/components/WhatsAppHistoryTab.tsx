...
type SelectionActionStatus = 'idle' | 'loading' | 'success' | 'error';

type SelectionActionState = {
  status: SelectionActionStatus;
  message?: string;
};
...
export default function WhatsAppHistoryTab({
  ...
  return (
    <>
      ...
      {selectionActionState.status !== 'idle' &&
        selectionActionState.status !== 'loading' &&
        selectionActionState.message && (
          <div
            className={`fixed bottom-6 right-6 z-50 w-full max-w-sm rounded-xl border px-4 py-3 shadow-lg transition-all ${
              selectionActionState.status === 'success'
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <button
                type="button"
                onClick={dismissSelectionFeedback}
                className="ml-2 rounded-full bg-white/20 p-1 text-white transition hover:bg-white/30"
