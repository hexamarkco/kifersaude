...
function DashboardKpiCard({
  ...
  const toneClass = tone === 'success'
    ? 'border-emerald-200/70 bg-[linear-gradient(180deg,rgba(236,253,245,0.96),rgba(255,255,255,0.96))]'
    : tone === 'warning'
      ? 'border-amber-200/80 bg-[linear-gradient(180deg,rgba(255,251,235,0.98),rgba(255,255,255,0.96))]'
      : 'border-[color:var(--panel-border-subtle,#e7dac8)] bg-[linear-gradient(180deg,var(--panel-surface,#fffdfa),color-mix(in_srgb,var(--panel-surface-soft,#f4ede3)_62%,var(--panel-surface,#fffdfa)))]';
  ...
  return (
    <div className={`rounded-3xl border px-4 py-4 shadow-sm ${toneClass}`}>
      ...
      <p className="mt-2 text-3xl font-semibold text-[color:var(--panel-text,#1a120d)]">{value}</p>
      <p className="mt-2 text-sm text-[color:var(--panel-text-soft,#5b4635)]">{helper}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-end justify-between gap-4">
        <div>
          <p className="text-sm font-semibold text-[color:var(--panel-text,#1a120d)]">{label}</p>
          <p className="text-xs text-[color:var(--panel-text-soft,#5b4635)]">{helper}</p>
        </div>
        <p className="text-sm font-semibold text-[color:var(--panel-text,#1a120d)]">{formatPercent(value)}</p>
      </div>
      <div className="h-2 rounded-full bg-[var(--panel-surface-soft,#f4ede3)]">
      ...
      <section className="rounded-[32px] border border-[var(--panel-border,#d4c0a7)] bg-[radial-gradient(circle_at_top_left,rgba(253,230,195,0.95),rgba(255,253,250,0.96)_48%,rgba(247,240,231,0.99)_100%)] p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:justify-between">
          <div className="max-w-3xl">
            ...
            <h3 className="mt-4 text-3xl font-semibold text-[color:var(--panel-text,#1a120d)]">Saúde do catálogo e operação do módulo</h3>
            <p className="mt-3 text-sm leading-6 text-[color:var(--panel-text-soft,#5b4635)]">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-[28px] border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--panel-text-muted,#876f5c)]">Cobertura do catálogo</p>
              <h4 className="mt-1 text-lg font-semibold text-[color:var(--panel-text,#1a120d)]">O que já está pronto para cotar</h4>
            </div>
          </div>
        ...
        <section className="rounded-[28px] border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--panel-text-muted,#876f5c)]">Cotações salvas</p>
              <h4 className="mt-1 text-lg font-semibold text-[color:var(--panel-text,#1a120d)]">Distribuição por modalidade</h4>
            </div>
          </div>
            ...
            {modalityBars.map((item) => (
              <div key={item.label} className="space-y-2">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-sm font-semibold text-[color:var(--panel-text,#1a120d)]">{item.label}</p>
                  <p className="text-sm text-[color:var(--panel-text-soft,#5b4635)]">{item.value}</p>
                </div>
                <div className="h-2 rounded-full bg-[var(--panel-surface-soft,#f4ede3)]">
      ...
      <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        <section className="rounded-[28px] border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--panel-text-muted,#876f5c)]">Qualidade da rede</p>
              <h4 className="mt-1 text-lg font-semibold text-[color:var(--panel-text,#1a120d)]">Cadastros hospitalares compartilhados</h4>
            </div>
          </div>
        ...
        <section className="rounded-[28px] border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[color:var(--panel-text-muted,#876f5c)]">Prioridades</p>
              <h4 className="mt-1 text-lg font-semibold text-[color:var(--panel-text,#1a120d)]">O que atacar primeiro</h4>
            </div>
          </div>
          ...
          <div className="mt-5 space-y-3">
            {[
              ...
              return (
                <div key={item.title} className="flex items-start gap-3 rounded-2xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface-soft,#f4ede3)] px-4 py-3">
                  <div className="rounded-2xl border border-[color:rgba(111,63,22,0.14)] bg-[var(--panel-surface,#fffdfa)] p-2 text-[var(--panel-accent-ink,#6f3f16)]">
                    <Icon className="h-4 w-4" />
                  </div>
                  <div>
                    <p className="text-sm font-semibold text-[color:var(--panel-text,#1a120d)]">{item.title}</p>
                    <p className="mt-1 text-sm text-[color:var(--panel-text-soft,#5b4635)]">{item.description}</p>
                  </div>
                </div>
              );
            })}
