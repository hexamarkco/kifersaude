...
CREATE OR REPLACE FUNCTION public.comm_whatsapp_restore_deleted_chat_on_new_inbound()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  IF OLD.deleted_at IS NOT NULL
     ...
     AND NEW.last_message_direction = 'inbound'
     AND NEW.last_message_at IS NOT NULL
     AND NEW.last_message_at > OLD.deleted_at
...
DROP TRIGGER IF EXISTS trg_comm_whatsapp_restore_deleted_chat_on_new_inbound ON public.comm_whatsapp_chats;
CREATE TRIGGER trg_comm_whatsapp_restore_deleted_chat_on_new_inbound
  BEFORE UPDATE ON public.comm_whatsapp_chats
  FOR EACH ROW
  EXECUTE FUNCTION public.comm_whatsapp_restore_deleted_chat_on_new_inbound();
