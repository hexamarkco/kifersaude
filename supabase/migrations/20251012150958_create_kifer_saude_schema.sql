...
CREATE TABLE IF NOT EXISTS leads (
  ...
  cidade text,
  regiao text,
  origem text NOT NULL, -- tráfego pago, Telein, indicação, orgânico
  tipo_contratacao text NOT NULL, -- Pessoa Física, MEI, CNPJ, Adesão
  operadora_atual text,
  status text NOT NULL DEFAULT 'Novo', -- Novo, Em contato, Cotando, Proposta enviada, Fechado, Perdido
...
CREATE TABLE IF NOT EXISTS contracts (
  ...
  status text NOT NULL DEFAULT 'Rascunho', -- Rascunho, Em análise, Documentos pendentes, Proposta enviada, Aguardando assinatura, Emitido, Ativo, Suspenso, Cancelado, Encerrado
  modalidade text NOT NULL, -- PF, MEI, CNPJ (PME), Adesão
  operadora text NOT NULL,
  produto_plano text NOT NULL,
  abrangencia text, -- Nacional, Regional
...
CREATE TABLE IF NOT EXISTS dependents (
  ...
  cpf text,
  data_nascimento date NOT NULL,
  relacao text NOT NULL, -- Cônjuge, Filho(a), Enteado(a), Pai/Mãe, Outro
  elegibilidade text,
...
CREATE TABLE IF NOT EXISTS documents (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  entity_type text NOT NULL, -- holder, dependent
  entity_id uuid NOT NULL,
  tipo_documento text NOT NULL, -- RG, CPF, Comprovante residência, CCMEI, Contrato social, Certidão, etc
...
CREATE TABLE IF NOT EXISTS reminders (
  ...
  contract_id uuid REFERENCES contracts(id) ON DELETE CASCADE,
  lead_id uuid REFERENCES leads(id) ON DELETE CASCADE,
  tipo text NOT NULL, -- Documentos pendentes, Assinatura, Ativação, Renovação, Retorno
