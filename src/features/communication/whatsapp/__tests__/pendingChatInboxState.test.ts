...
const baseChat = (overrides: Partial<CommWhatsAppChat> = {}): CommWhatsAppChat => ({
  ...
  merged_into_chat_id: overrides.merged_into_chat_id ?? null,
  lead_link_source: overrides.lead_link_source ?? null,
  lead_linked_at: overrides.lead_linked_at ?? null,
...
test('keeps archive patch when outbound activity reaches the server', () => {
  const state = new Map<string, PendingChatInboxStatePatch>();
  const activeChat = baseChat({ is_archived: false, archived_at: null });
  const archivedAt = new Date(Date.now() - 1_000).toISOString();
  const patch: PendingChatInboxStatePatch = {
