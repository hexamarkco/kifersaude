-- comm_whatsapp_persist_message vinha derrubando 100% das mensagens inbound
-- reais desde que comm_whatsapp_resolve_canonical_chat_uuid ganhou o fallback
-- de variante do 9o digito brasileiro (20260912190000_reconcile_comm_whatsapp
-- _phone_variant_chats.sql). O wrapper resolve e trava a identidade canonica
-- ANTES de chamar comm_whatsapp_persist_message_internal, mas o _internal
-- ainda faz seu proprio lookup por igualdade exata de external_chat_id — sem
-- usar esse fallback. Quando a mensagem real chega no formato que so resolve
-- via variante do 9 (ou LID), essa checagem de consistencia (adicionada em
-- 20260912129000_harden_comm_whatsapp_identity_model.sql para pegar bugs de
-- concorrencia) acusa divergencia e da RAISE EXCEPTION — o webhook devolve
-- 500, a mensagem nunca e persistida, e a Whapi nao reentrega o evento. Ou
-- seja: a trava desenhada para proteger contra corrupcao silenciosa estava
-- causando perda silenciosa de mensagem real, de forma deterministica (nao
-- rara), a cada inbound de um numero cuja variante de digito ja existia como
-- outro chat no canal.
--
-- Correcao: em vez de abortar a persistencia inteira quando as duas
-- resolucoes divergem, registra o conflito em comm_whatsapp_identity_conflicts
-- (pra investigar/reconciliar depois, mesmo padrao ja usado para
-- reverse_mapping_conflict) e SEGUE usando o chat que o _internal realmente
-- gravou (fonte da verdade: e o unico FOR UPDATE que roda no mesmo instante
-- da escrita). Continuar com esse chat real e estritamente melhor do que
-- forcar tudo para v_canonical_chat_id, que nesse cenario e um chat que a
-- mensagem nunca chegou a tocar.
...
BEGIN
  ...
  -- Antes: RAISE EXCEPTION aqui perdia a mensagem inteira (webhook 500, sem
  -- reentrega da Whapi) sempre que a resolucao com fallback de variante do 9
  -- divergia do lookup exato feito dentro do _internal. Agora so registra o
  -- conflito para investigacao e segue com o chat que o _internal realmente
  -- gravou (dedupe_key evita reexplodir a tabela em reenvios/retries).
