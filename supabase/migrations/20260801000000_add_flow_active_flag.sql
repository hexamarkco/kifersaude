/*
  Per-flow active toggle support (ativo)

  - check_auto_contact_inactivity_triggers only scans flows with
    COALESCE(flow->>'ativo', 'true') != 'false'
  - automation_flows_health() exposes the ativo flag per flow
*/
