...
REVOKE ALL ON FUNCTION public.comm_whatsapp_persist_message(
  uuid, text, text, text, text, text, text, timestamptz, boolean, text, text, text, text, text, uuid, text, text, text, timestamptz, text, jsonb
) FROM PUBLIC;
...
BEGIN
  ...
  UPDATE public.comm_whatsapp_messages
  SET
    delivery_status = COALESCE(v_delivery_status, public.comm_whatsapp_messages.delivery_status),
    status_updated_at = COALESCE(p_status_updated_at, now()),
    error_message = CASE
      WHEN v_error_message IS NOT NULL THEN v_error_message
      WHEN COALESCE(v_delivery_status, public.comm_whatsapp_messages.delivery_status) IN ('sent', 'delivered', 'read', 'played', 'received') THEN NULL
...
REVOKE ALL ON FUNCTION public.comm_whatsapp_update_message_status(uuid, text, text, timestamptz, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.comm_whatsapp_update_message_status(uuid, text, text, timestamptz, text) TO authenticated, service_role;
