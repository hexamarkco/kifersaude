...
-- Resolve primeiro pelos telefones já normalizados e indexados. A comparação
-- completa de variantes continua disponível como fallback, mas não deve ser
-- executada para cada contato quando já existe uma correspondência direta.
...
AS $$
  ...
  indexed_matches AS MATERIALIZED (
    ...
    JOIN public.comm_whatsapp_chat_identifiers identifier
     ...
     AND (
       contact.contact_id = identifier.external_chat_id
       OR public.normalize_comm_whatsapp_chat_id(contact.contact_id) = identifier.external_chat_id
     )
    WHERE contact.channel_id = p_channel_id
      AND contact.saved = true
      AND public.comm_whatsapp_is_valid_display_name(contact.display_name)
