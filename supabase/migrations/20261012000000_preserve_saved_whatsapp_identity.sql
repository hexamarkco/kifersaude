...
-- A atualização de mensagem pode ocorrer enquanto o cache de contatos está
-- sendo sincronizado. Nesse intervalo não existe uma linha `saved = true`
-- para o chat, e a rotina de identidade não pode apagar um nome que já foi
-- salvo explicitamente no Inbox.
...
-- A lista do Inbox deve ter a mesma fonte de identidade da thread. O fallback
-- é aplicado apenas depois da paginação para não transformar cada refetch em
-- uma varredura completa do cache de contatos.
...
AS $$
  ...
  SELECT
    ...
    c.phone_digits,
    COALESCE(
      CASE WHEN saved_contact.is_manual THEN saved_contact.display_name END,
    ...
    WHERE contact.channel_id = c.channel_id
      ...
      AND (
        public.comm_whatsapp_phone_lookup_keys(contact.phone_digits)
          && public.comm_whatsapp_phone_lookup_keys(c.phone_digits)
        OR EXISTS (
          SELECT 1
          FROM public.comm_whatsapp_chat_identifiers identifier
          WHERE identifier.chat_id = c.id
            AND identifier.channel_id = c.channel_id
            AND identifier.external_chat_id = public.normalize_comm_whatsapp_chat_id(contact.contact_id)
        )
      )
    ORDER BY
