...
SET status = lsc.nome
FROM public.lead_status_config lsc
WHERE l.status_id = lsc.id
  AND l.status IS DISTINCT FROM lsc.nome;
...
AS $$
  SELECT
    ...
    l.telefone,
    l.observacoes,
    COALESCE(lsc.nome, l.status) AS status_nome,
    COALESCE(lsc.nome, l.status) AS status_value,
