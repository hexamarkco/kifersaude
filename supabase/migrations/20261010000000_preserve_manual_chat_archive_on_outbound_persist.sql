-- O arquivamento da Inbox e uma acao manual. Uma persistencia outbound pode
-- acontecer depois que o usuario arquivou a conversa (por exemplo, o eco do
-- envio ou uma confirmacao de status). A funcao interna historicamente usada
-- por esta RPC tambem reabre chats outbound novos; neste wrapper, preservamos
-- o estado arquivado observado sob lock antes de delegar a persistencia.
-- Mensagens inbound novas continuam podendo desarquivar a conversa.
...
BEGIN
  ...
  LOOP
    SELECT
      ...
      chat.is_archived,
      chat.archived_at
    INTO v_next_chat_id, v_resolved_external_chat_id, v_phone_number, v_was_archived, v_archived_at
  ...
  -- Sempre retornar o chat_id canonico, mesmo em caso de divergencia de
  -- identidade. Isso garante que o frontend usa o chat correto.
