-- Otimizacao do predicado sticky de resposta: o CTE outbound_activity deixa
-- de avaliar comm_whatsapp_message_preview_text em TODAS as mensagens outbound
-- (7.2s -> 22ms; so ~1.2% das outbounds sao tecnicas/action/system/unknown).
-- O filtro de preview permanece no lado inbound do EXISTS (poucas linhas por
-- lead via indice chat_direction_message_at), preservando a semantica:
-- "respondido" = inbound real posterior a 1a outbound do lead.
