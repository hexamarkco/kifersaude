...
import { Checkbox, Dialog, DialogBody, DialogDescription, DialogFooter, DialogHeader, DialogTitle, Field, Input, Surface, Button } from '../../../design-system';
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()} size="lg">
      <DialogHeader onClose={onClose}>
        ...
        <DialogDescription>Gere um PDF para enviar ao cliente ou crie um link público com os planos cotados.</DialogDescription>
      </DialogHeader>
      <DialogBody>
        <div className="space-y-5">
          <Surface variant="muted" padding="sm">
            <label className="flex items-start gap-3">
              <Checkbox checked={includeNetworkComparison && hasNetworkComparison} onChange={(event) => setIncludeNetworkComparison(event.target.checked)} disabled={!hasNetworkComparison} />
              <span><span className="block text-sm font-semibold text-[var(--text-primary)]">Incluir comparativo de rede</span><span className="mt-1 block text-sm text-[var(--text-muted)]">{hasNetworkComparison ? 'Inclui a matriz com os prestadores compartilhados e diferenciais entre os planos.' : 'Nenhum dos planos selecionados possui rede hospitalar cadastrada para comparação.'}</span></span>
            </label>
          </Surface>
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
            <Field label="Link público">
              <Input value={shareLink} onChange={() => undefined} readOnly placeholder={creatingShareLink ? 'Preparando link público da cotação...' : 'O link público será exibido automaticamente aqui.'} />
            </Field>
            <div className="flex flex-wrap gap-2">
              <Button variant="secondary" onClick={handleCopyLink} disabled={!shareLink || creatingShareLink}><Copy className="h-4 w-4" />Copiar</Button>
              <Button variant="secondary" onClick={() => window.open(shareLink, '_blank', 'noopener,noreferrer')} disabled={!shareLink || creatingShareLink}><ExternalLink className="h-4 w-4" />Abrir</Button>
              <Button variant="secondary" onClick={handleRefreshLink} loading={creatingShareLink} disabled={exportingPdf || selectedItems.length === 0}>Atualizar link</Button>
            </div>
          </div>
          <Surface padding="sm"><p className="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">Prévia do compartilhamento</p><p className="mt-2 text-sm text-[var(--text-muted)]">O cliente recebe os {selectedItems.length} plano(s) atualmente selecionados na cotação, com visual pronto para apresentação.</p></Surface>
        </div>
      </DialogBody>
      <DialogFooter>
