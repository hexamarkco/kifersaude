-- Adiciona filtro por agente responsável (do lead vinculado) à listagem de
-- chats do Inbox. Não existe hoje nenhum conceito de "dono" no chat em si —
-- só o lead vinculado (quando existe) tem responsavel_id — então o filtro
-- reaproveita esse dado em vez de introduzir atribuição própria de chat
-- (decisão registrada: chats sem lead vinculado ficam fora deste filtro).
...
-- leads.responsavel_id já é usado em outras RPCs, mas não havia índice
-- dedicado — o filtro acima faz igualdade direta sobre essa coluna.
