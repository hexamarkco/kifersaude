...
BEGIN
  ...
  LOOP
    CASE v_hist.sn
      ...
      WHEN 'aguardando cotação' THEN IF 4 > v_best_order THEN v_best_order := 4; v_best_stage := 'Aguardando cotacao'; END IF;
      ...
      WHEN 'decisão' THEN IF 6 > v_best_order THEN v_best_order := 6; v_best_stage := 'Decisao'; END IF;
      WHEN 'contratacao' THEN IF 7 > v_best_order THEN v_best_order := 7; v_best_stage := 'Contratacao'; END IF;
      WHEN 'contratação' THEN IF 7 > v_best_order THEN v_best_order := 7; v_best_stage := 'Contratacao'; END IF;
...
console.log('Deploying fixed classify function...');
...
const { data: test } = await supabase.rpc('audit_classify_single_lead', {
  p_lead_id: '977278ec-82a0-42e1-8a62-e59fb98f582b',
  p_perdido_id: 'e6dfc1b0-720d-446a-8ed1-d773f781bbba',
  p_reativacao_id: 'c6131bfc-9d6a-430e-af7c-44f5d6731186'
});
if (test?.length) {
  console.log('Test (Aline):', JSON.stringify(test[0], null, 2));
  console.log('furthest_stage:', test[0].furthest_stage);
}
