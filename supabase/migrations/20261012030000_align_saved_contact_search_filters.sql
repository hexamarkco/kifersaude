...
-- A listagem do Inbox pode usar o cache de contatos como fallback enquanto a
-- identidade do chat ainda está sendo atualizada. Busca e filtros precisam
-- considerar a mesma fonte, ou o contato pode aparecer com o nome salvo e,
-- ao mesmo tempo, não ser encontrado por esse nome.
      ...
      AND (
        ...
        OR EXISTS (
          ...
          WHERE contact.channel_id = c.channel_id
            ...
            AND contact.saved = true
            AND public.comm_whatsapp_is_valid_display_name(contact.display_name)
            AND contact.display_name ILIKE '%' || input.search_text || '%'
        )
      )
