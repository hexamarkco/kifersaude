...
export default function ContractDetails({
  ...
  const [editingHolder, setEditingHolder] = useState<ContractHolder | null>(
    null,
  );
  ...
  const getContractStartDate = () => {
    const start =
      parseDate(contract.data_inicio) || parseDate(contract.created_at);
  ...
  const getAgeAdjustmentAlerts = () => {
    ...
    const holdersToAdjust = holders.filter((holder) => {
      ...
      return (
        ...
        AGE_ADJUSTMENT_MILESTONES.includes(ageThisYear)
      );
    });
  ...
  const upcomingAgeAdjustments = useMemo(() => {
    ...
    const getNextMilestone = (birthDate: Date) => {
      for (const age of AGE_ADJUSTMENT_MILESTONES) {
        ...
        if (
          targetDate > today &&
          (!contractStart || targetDate > contractStart)
        ) {
  ...
  const bonusEligibleLivesFromRecords =
    totalLivesInRecords > 0
      ? holders.filter(
          (holder) =>
        ...
        dependents.filter(
          (dependent) =>
            dependent.bonus_por_vida_aplicado ??
            contract.bonus_por_vida_aplicado ??
            true,
        ).length
      : 0;
  const defaultBonusLives =
    totalLivesInRecords > 0
  ...
  const hasSignupFeeConfig =
    isAdesaoContract(contract.modalidade) &&
    (contract.taxa_adesao_tipo === "percentual_mensalidade" ||
      contract.taxa_adesao_tipo === "valor_fixo");
    ...
    return (
      <div className="space-y-2">
        {docs.map((doc) => (
          <div
            key={doc.id}
            className={`${detailPanelClass} flex items-center justify-between px-3 py-2 text-sm`}
          >
            <div className="min-w-0">
              <p className={`${detailHeadingTextClass} truncate`}>
                {doc.tipo_documento}
              </p>
              <p className={`${detailMutedTextClass} truncate`}>
                {doc.nome_arquivo}
              </p>
            </div>
            <a
      ...
      <div className="p-1">
              ...
              <div className="space-y-2">
                {contractTimeline.map((event) => (
                  ...
                  >
                    ...
                    <div className={detailMutedTextClass}>
          ...
          {contract.comissao_prevista && (
            <div className={detailDividerClass}>
              <div className="flex items-center justify-between">
                ...
                <span className={detailMetricValueClass}>
                  R${" "}
                  {contract.comissao_prevista.toLocaleString("pt-BR", {
                    minimumFractionDigits: 2,
                  })}
                </span>
              </div>
                  ...
                  {commissionInstallments.length > 0 ? (
                    <div className="comm-card comm-card-warning space-y-2 p-3">
                      {commissionSummary.installments.map((parcel, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span>
                              R${" "}
                              {parcel.resolvedValue.toLocaleString("pt-BR", {
                                minimumFractionDigits: 2,
                              })}
                            </span>
                          </div>
                      ...
                      <div className="flex items-center justify-between border-t border-[var(--panel-border-subtle,#e7dac8)] pt-2 text-xs text-[var(--panel-text-soft,#5b4635)]">
                        ...
                        <span className={detailHeadingTextClass}>
                          R${" "}
                          {commissionSummary.totalResolvedValue.toLocaleString(
                            "pt-BR",
                            {
                              minimumFractionDigits: 2,
                            },
              ...
              <div className="comm-card comm-card-brand p-4">
                {bonusSummary.hasConfigurations ? (
                  ...
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <span className={detailMetricValueClass}>
                      R${" "}
                      {(bonusSummary.legacyValue || 0).toLocaleString("pt-BR", {
                        minimumFractionDigits: 2,
                      })}
                    </span>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <span className={detailMutedTextClass}>
                    Vidas no contrato:
                  </span>
                  <span className={detailBodyStrongClass}>
                    {contract.vidas || 1}
        ...
        <div className="mb-6">
          ...
          {holders.length > 0 ? (
              ...
              {holders.map((holderItem) => (
                <div key={holderItem.id} className={`${detailPanelClass} p-4`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <h5 className={`mb-3 ${detailHeadingTextClass}`}>
                        {holderItem.nome_completo}
                      </h5>
                      <div
            ...
            <div className={detailEmptyStateClass}>
              ...
              <p className={`${detailBodyTextClass} mb-3`}>
        ...
        <div className="mb-6">
            ...
            {canEditContracts && (
              <Button
                onClick={() => {
                  if (holders.length === 0) {
                    toast.warning(
                      "Cadastre um titular antes de adicionar dependentes.",
                    );
                    return;
                  }
          ...
          {holders.length > 0 ? (
            <div className="space-y-4">
              {holders.map((holderItem) => {
                const holderDependents = dependents.filter(
                  (d) => d.holder_id === holderItem.id,
                );
                return (
                    ...
                    <div className="space-y-3 p-4">
                      {holderDependents.length > 0 ? (
                        holderDependents.map((dependent) => (
                          <div
                            key={dependent.id}
                            className={`${detailNestedPanelClass} flex items-start justify-between gap-4 p-3`}
                          >
                            <div className="flex-1">
                              <h5 className={`mb-1 ${detailHeadingTextClass}`}>
                                {dependent.nome_completo}
                              </h5>
                              <div
                                className={`grid grid-cols-2 gap-3 text-sm md:grid-cols-3 ${detailBodyTextClass}`}
                              >
                        ...
                        <p className={detailBodyTextClass}>
                          Nenhum dependente para este titular.
                        </p>
                      )}
                    </div>
                  </div>
            ...
            <div className={detailEmptyStateClass}>
              ...
              <p className={detailBodyTextClass}>
                Cadastre um titular para incluir dependentes.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-4">
            {holders.map((holderItem) => (
              <div
                key={`docs-holder-${holderItem.id}`}
                className={`${detailPanelSoftClass} p-4`}
              >
                <p className={`mb-2 ${detailBodyStrongClass}`}>
                  Titular: {holderItem.nome_completo}
                </p>
            ...
            {dependents.map((dependent) => (
              <div
                key={`docs-dependent-${dependent.id}`}
                className={`${detailPanelSoftClass} p-4`}
              >
                <p className={`mb-2 ${detailBodyStrongClass}`}>
                  Dependente: {dependent.nome_completo}
                </p>
          ...
          {auditEvents.length === 0 ? (
            ...
            <div className="space-y-2">
              {auditEvents.map((event) => (
                <div
                  key={event.id}
                  className={`${detailNestedPanelClass} flex items-start justify-between gap-3 px-3 py-2 text-sm`}
                >
                  <div className="min-w-0">
                    <p className={detailBodyStrongClass}>{event.label}</p>
                    {event.description && (
                      <p className={`truncate ${detailMutedTextClass}`}>
                        {event.description}
                      </p>
                    )}
                  </div>
          ...
          {interactions.length === 0 ? (
            ...
            <div className="space-y-3">
              {interactions.map((interaction) => (
                <div
                  key={interaction.id}
                  className={`${detailNestedPanelClass} p-4`}
                >
                  <div className="mb-2 flex items-start justify-between gap-3">
                    ...
                    <div
                      className={`flex items-center gap-3 text-sm ${detailMutedTextClass}`}
                    >
                      <span>
                        {new Date(
                  ...
                  <p className={detailBodyStrongClass}>
