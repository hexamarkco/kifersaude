...
CREATE INDEX IF NOT EXISTS idx_comm_whatsapp_chats_archived_last_message
  ON public.comm_whatsapp_chats (is_archived, last_message_at DESC NULLS LAST, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_comm_whatsapp_chats_muted
  ON public.comm_whatsapp_chats (is_muted);
...
REVOKE ALL ON FUNCTION public.comm_whatsapp_update_chat_inbox_state(uuid, boolean, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.comm_whatsapp_update_chat_inbox_state(uuid, boolean, boolean) TO authenticated;
...
BEGIN
  ...
  SET
    ...
    END,
    is_archived = CASE
      WHEN COALESCE(p_increment_unread, false) AND v_inserted THEN false
      ELSE public.comm_whatsapp_chats.is_archived
    END,
    archived_at = CASE
      WHEN COALESCE(p_increment_unread, false) AND v_inserted THEN NULL
