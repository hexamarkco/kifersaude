...
const assistantToolLabel = (name?: string) => {
  if (name === 'load_inbox_summary') return 'Resumo do inbox';
  if (name === 'load_selected_chat_context') return 'Chat selecionado';
  if (name === 'search_whatsapp_history') return 'Histórico WhatsApp';
  if (name === 'search_cotador_quotes') return 'Cotações Cotador';
  if (name === 'prepare_draft_message') return 'Rascunho';
  if (name === 'prepare_bulk_whatsapp_send') return 'Disparo selecionável';
  return name || 'Tool';
};

const assistantToolStatusLabel = (status?: string) => {
          ...
          <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-3 pr-2 sm:p-4 sm:pr-3">
            ...
            {response && !loading ? (
              <div className="min-h-0 space-y-3 rounded-3xl border border-orange-200/15 bg-orange-950/15 p-3">
                ...
                {response.assistantTools ? (
                  <div className="space-y-2 rounded-2xl border border-orange-200/15 bg-black/20 px-3 py-2.5">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-orange-50">Tools tipadas</p>
                        <p className="mt-1 text-xs leading-4 text-orange-100/55">
                          Leituras executadas: {response.assistantTools.audit?.executedReadTools ?? 0} · Ações preparadas: {response.assistantTools.audit?.preparedActionTools ?? 0}
                        </p>
                      </div>
                      <span className="rounded-full border border-orange-200/15 px-2.5 py-1 text-[11px] font-semibold text-orange-100/65">
                        v{response.assistantTools.version ?? 1}
                      </span>
                    </div>
                    <div className="grid gap-2 sm:grid-cols-2">
                      {(response.assistantTools.calls ?? []).slice(0, 6).map((call) => (
                        <div key={call.id || call.name} className="rounded-xl border border-orange-200/10 bg-orange-300/5 px-3 py-2">
                          <div className="flex items-center justify-between gap-2">
                            <p className="truncate text-xs font-semibold text-orange-50">{assistantToolLabel(call.name)}</p>
                            <span className={cx(
                              'rounded-full px-2 py-0.5 text-[10px] font-semibold',
                              call.status === 'executed' ? 'bg-emerald-300/10 text-emerald-100' : call.status === 'prepared' ? 'bg-amber-300/10 text-amber-100' : 'bg-stone-300/10 text-orange-100/55',
                            )}
                            >
                              {assistantToolStatusLabel(call.status)}
                            </span>
                          </div>
                          {call.evidencePolicy ? (
                            <p className="mt-1 line-clamp-2 text-xs leading-4 text-orange-100/55">{call.evidencePolicy}</p>
                          ) : null}
                          {call.requiresConfirmation ? (
                            <p className="mt-1 text-[11px] leading-4 text-amber-100/75">Exige confirmação humana.</p>
                          ) : null}
                        </div>
                      ))}
                    </div>
                    {response.assistantTools.audit?.note ? (
                      <p className="text-xs leading-4 text-orange-100/50">{response.assistantTools.audit.note}</p>
                    ) : null}
                  </div>
                ) : null}
