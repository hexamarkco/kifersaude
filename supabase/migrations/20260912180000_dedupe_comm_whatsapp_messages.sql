-- Defesa contra mensagens duplicadas de chats antigos no Inbox WhatsApp.
--
-- Contexto: mensagens que chegam sem `external_message_id` (Webhook/Sync da
-- Whapi sem id no payload) eram inseridas SEMPRE no branch sem id do
-- `comm_whatsapp_persist_message_internal`, sem qualquer verificacao de
-- duplicidade por conteudo. Resyncs de historico antigo re-inseriam as mesmas
-- mensagens digitadas manualmente, criando blocos duplicados (ex.: toda a
-- conversa do dia reaparecendo novamente no chat).
--
-- Dois ajustes:
--   1. `comm_whatsapp_persist_message_internal` passa a ignorar insercao
--      quando ja existe mensagem com MESMO conteudo no MESMO chat/direcao/tipo
--      (media comparada por `media_id`, texto por `text_content`), valendo
--      apenas para mensagens SEM id externo (as com id continuam deduplicadas
--      pelo indice unico (channel_id, external_message_id)). Envios reais
--      (operador e automacoes) sempre persistem COM id e nunca sao afetados.
--   2. Limpeza unica das duplicatas historicas: remove apenas linhas SEM id
--      externo que repetem o conteudo de outra mensagem anterior no mesmo
--      chat/direcao/tipo, preservando a mais antiga e nunca tocando linhas
--      referenciadas por outras tabelas (opt-outs, sugestoes IA e jobs de
--      enriquecimento — unicas FKs apontando para comm_whatsapp_messages(id)).
...
BEGIN
  ...
  IF v_external_message_id IS NOT NULL THEN
    ...
    END IF;
  ELSE
    -- Mensagens sem id externo: ignora insercao quando ja existe conteudo
    -- identico no mesmo chat/direcao/tipo. Media e comparada por media_id;
    -- texto apenas para message_type = 'text' (resumos de midia como
    -- '[Documento]' nao sao segurancas suficientes para deduplicar).
    v_message_id := NULL;
    v_inserted := false;
...
-- ---------------------------------------------------------------------------
-- Limpeza unica: remove duplicatas historicas de mensagens SEM id externo.
-- Preserva a mensagem mais antiga de cada grupo de conteudo identico e nunca
-- apaga linhas referenciadas por outras tabelas.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  ...
  v_tables text[][] := ARRAY[
    ...
    ARRAY['public.comm_whatsapp_ai_intent_suggestions', 'message_id'],
    ARRAY['public.comm_whatsapp_enrichment_jobs', 'message_id']
  ];
BEGIN
  FOREACH v_table_record SLICE 1 IN ARRAY v_tables LOOP
    IF
      to_regclass(v_table_record[1]) IS NOT NULL
      AND EXISTS (
        SELECT 1
        FROM information_schema.columns
        WHERE table_schema = split_part(v_table_record[1], '.', 1)
          AND table_name = split_part(v_table_record[1], '.', 2)
          AND column_name = v_table_record[2]
      )
    THEN
      EXECUTE format(
        'SELECT COALESCE(array_agg(DISTINCT %I), ''{}'') FROM %s',
        v_table_record[2],
        v_table_record[1]
      ) INTO v_ids;
      v_protected_ids := v_protected_ids || COALESCE(v_ids, '{}');
    END IF;
  END LOOP;

  DELETE FROM public.comm_whatsapp_messages AS target
  WHERE target.external_message_id IS NULL
    AND NOT (target.id = ANY(v_protected_ids))
    AND EXISTS (
      SELECT 1
      FROM public.comm_whatsapp_messages AS older
      WHERE older.chat_id = target.chat_id
        AND older.direction = target.direction
        AND older.message_type = target.message_type
        AND older.id <> target.id
        AND (
          (target.media_id IS NOT NULL AND older.media_id IS NOT NULL AND older.media_id = target.media_id)
          OR (
            target.media_id IS NULL
            AND target.message_type = 'text'
            AND NULLIF(btrim(COALESCE(target.text_content, '')), '') IS NOT NULL
            AND older.media_id IS NULL
            AND NULLIF(btrim(COALESCE(older.text_content, '')), '') IS NOT NULL
            AND btrim(older.text_content) = btrim(target.text_content)
          )
        )
        AND (older.message_at, older.id) < (target.message_at, target.id)
    );
END
$$;
