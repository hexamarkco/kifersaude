...
export const normalizeCommissionInstallments = (
  ...
  return installments
    .map((item) => {
      const percentual = typeof item?.percentual === 'number' && Number.isFinite(item.percentual)
        ? item.percentual
        : undefined;
    ...
    .filter((item): item is ContractCommissionInstallment => Boolean(item));
};

export const getCommissionInstallmentSummary = (
  contract: Pick<Contract, 'comissao_prevista' | 'comissao_parcelas' | 'mensalidade_total'>
) => {
  const installments = normalizeCommissionInstallments(contract.comissao_parcelas);
  const totalCommission = contract.comissao_prevista || 0;
  const totalByValue = installments.reduce((sum, item) => sum + (item.valor || 0), 0);
  const totalPercent = installments.reduce((sum, item) => sum + (item.percentual || 0), 0);
  ...
  return {
    ...
    totalByValue,
    totalPercent,
    totalResolvedValue: resolvedInstallments.reduce((sum, item) => sum + item.resolvedValue, 0),
  };
};
