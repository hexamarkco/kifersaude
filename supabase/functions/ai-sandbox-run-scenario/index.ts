...
const buildJudgePrompt = (history: SandboxMessageRow[], handoffTriggered: boolean): { systemPrompt: string; userPrompt: string } => {
  ...
  const userPrompt = [
    ...
    '6. Se o lead reclamou do plano atual ou pediu cancelamento, o atendente demonstrou empatia E ofereceu buscar uma opcao de plano melhor (oportunidade de venda) antes de so acionar handoff? So deveria ir direto pro handoff se o lead recusou a nova cotacao ou so quis mesmo cancelar/reclamar.',
