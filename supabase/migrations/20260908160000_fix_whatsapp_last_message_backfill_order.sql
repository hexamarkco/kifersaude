with preview_candidates as (
  select
    ...
    case
      ...
      when lower(coalesce(wm.type, '')) in ('video', 'short', 'gif') then '[Video]'
      when lower(coalesce(wm.type, '')) in ('audio', 'voice', 'ptt') then '[Audio]'
      when lower(coalesce(wm.type, '')) = 'document' then '[Documento]'
      when lower(coalesce(wm.type, '')) = 'contact' then '[Contato]'
      when lower(coalesce(wm.type, '')) in ('location', 'live_location') then '[Localizacao]'
