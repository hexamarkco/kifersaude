...
if not exist "%~dp0scripts\run-supabase-migrations.sh" (
  echo [ERRO] Script nao encontrado: scripts\run-supabase-migrations.sh
  set "EXIT_CODE=1"
  goto :end
)
...
call wsl -e bash -lc "cd \"%WSL_REPO_PATH%\" && bash scripts/run-supabase-migrations.sh"
