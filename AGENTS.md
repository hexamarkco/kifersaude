...
- 2026-05-25: As funcoes de permissao `current_user_is_access_admin()` e `current_user_access_role()` foram refatoradas para `LANGUAGE plpgsql` com cache transacional via `set_config('app.rls_*')`, eliminando a execucao de queries SQL por linha durante a avaliacao de RLS. A funcao `current_user_can_edit_whatsapp()` passou a delegar para `current_user_is_access_admin()` em vez de usar `LEFT JOIN` com `role='admin'` hardcoded. Políticas RLS em tabelas quentes (whatsapp_chats, whatsapp_messages, whatsapp_campaign_import_jobs, blog_posts, storage.objects, etc.) tiveram a subquery inline `EXISTS (SELECT 1 FROM user_profiles WHERE role = 'admin')` substituida por `public.current_user_is_access_admin()`, que é cacheada por transacao. Ver migration `20260911405000_rls_performance_cache_refactor.sql`.
