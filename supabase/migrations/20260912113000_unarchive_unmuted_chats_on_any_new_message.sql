-- Preserve the established persistence implementation and extend only the
-- archive transition: a real new inbound or outbound message reopens unmuted chats.
ALTER FUNCTION public.comm_whatsapp_persist_message(
...
) RENAME TO comm_whatsapp_persist_message_internal;

CREATE FUNCTION public.comm_whatsapp_persist_message(
  p_channel_id uuid,
...
BEGIN
  ...
  THEN
    ...
    WHERE id = v_result.chat_id
      AND is_archived
      AND NOT is_muted
      AND (archived_at IS NULL OR v_message_at > archived_at)
    RETURNING unread_count INTO v_unread_count;
  END IF;
