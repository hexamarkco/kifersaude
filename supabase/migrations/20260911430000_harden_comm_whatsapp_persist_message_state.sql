...
SET
  ...
  status_updated_at = GREATEST(COALESCE(status_updated_at, '-infinity'::timestamptz), COALESCE(message_at, created_at, now())),
  error_message = NULL
WHERE direction = 'outbound'
  AND external_message_id IS NOT NULL
  AND lower(COALESCE(delivery_status, '')) IN ('pending', 'queued', 'sending');

UPDATE public.comm_whatsapp_send_requests
SET
  status = CASE WHEN lower(COALESCE(status, '')) = 'sending' THEN 'completed' ELSE status END,
  delivery_status = 'sent',
  error_message = NULL,
  updated_at = now()
WHERE external_message_id IS NOT NULL
  AND lower(COALESCE(delivery_status, status, '')) IN ('pending', 'queued', 'sending')
