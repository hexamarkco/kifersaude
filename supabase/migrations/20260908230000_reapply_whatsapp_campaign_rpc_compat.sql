/*
  # Reapply WhatsApp campaign RPC compatibility

  Some environments still run older RPC definitions for WhatsApp campaigns.
  Those versions write deprecated text columns on `public.leads`
  (`origem`, `status`, `responsavel`, `tipo_contratacao`) and fail with:

    column "origem" of relation "leads" does not exist

  This migration re-applies the filter resolver and campaign creation RPCs so
  they only depend on the current ID-based lead schema.
*/
