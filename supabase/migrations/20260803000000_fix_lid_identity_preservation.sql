/*
  Fix: preserve previously resolved phone numbers for @lid chats

  The persist wrapper unconditionally cleared phone_number/phone_digits on
  every inbound @lid message (lines 452-454).  An enrichment run that resolved
  a LID → phone was undone as soon as the next message arrived.
  
  Now the phone columns are only set to '' when they were already empty,
  preserving any prior identity resolution.
*/
...
BEGIN
  ...
  -- FIXED: preserve previously resolved phone numbers
  IF v_is_lid AND v_phone_number IS NULL THEN
    UPDATE public.comm_whatsapp_chats AS chat
    SET
        phone_number = CASE WHEN NULLIF(btrim(chat.phone_number), '') IS NULL THEN '' ELSE chat.phone_number END,
        phone_digits = CASE WHEN NULLIF(btrim(chat.phone_digits), '') IS NULL THEN '' ELSE chat.phone_digits END,
        display_name = CASE
