-- 2026-05-20: Restringe o desarquivamento automatico de chats do WhatsApp para
-- corrigir o bug onde uma conversa arquivada retorna sozinha para a Inbox.
--
-- Antes desta migration o RPC `comm_whatsapp_persist_message` desarquivava
-- qualquer chat nao mutado sempre que uma mensagem nova fosse inserida -
-- inclusive em re-sincronizacoes manuais (historico antigo), envios outbound
-- da propria UI e quaisquer eventos do webhook (status, ack, reactions etc.).
--
-- A partir de agora o desarquivamento automatico so acontece quando todas as
-- condicoes abaixo forem verdadeiras:
--   1. A mensagem foi recem inserida (`v_inserted = true`).
--   2. A mensagem e inbound real (a unica fonte que pede increment_unread =
--      `comm-whatsapp-webhook` ao receber mensagens novas do contato).
--   3. A direcao registrada e `inbound`.
--   4. O chat nao esta silenciado (`is_muted = false`).
--   5. A mensagem e mais recente que a marcacao de arquivamento
--      (`v_message_at > archived_at`).
--
-- Isso preserva o comportamento natural ("contato respondeu - chat volta
-- para a inbox") sem que sincronizacoes ou nossos proprios envios
-- desarquivem o chat indesejadamente.
...
BEGIN
  ...
  SET
    ...
    END,
    -- BUG FIX (2026-05-20): desarquivar APENAS quando:
    --   * mensagem recem inserida (v_inserted)
    --   * inbound real (increment_unread = true E direcao = inbound)
    --   * chat nao mutado
    --   * mensagem mais recente que o arquivamento atual
    -- Isso evita que syncs manuais, envios outbound da propria UI ou eventos
    -- de status (acks, reactions) revertam o arquivamento feito pelo usuario.
    is_archived = CASE
      WHEN v_inserted
        ...
        AND (
          public.comm_whatsapp_chats.archived_at IS NULL
