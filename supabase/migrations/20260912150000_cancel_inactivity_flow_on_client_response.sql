-- Regra operacional definitiva do "sem resposta" dos fluxos de inatividade:
--
-- Uma vez que o cliente RESPONDEU (mensagem inbound real — visivel, sem ruido
-- tecnico — posterior a 1a mensagem outbound real do lead), o fluxo de
-- inatividade e CANCELADO (jobs pending viram skipped) e o lead nunca mais e
-- elegivel para novas execucoes, mesmo que o operador responda depois.
--
-- Predicado sticky: EXISTS(inbound real com message_at > first_outbound_real).
-- Mensagens tecnicas (preview NULL via comm_whatsapp_message_preview_text,
-- ex.: "[Mensagem]"/type unknown) nao contam como resposta.
...
BEGIN
  BEGIN
    ...
    LOOP
      ...
      -- responderam alguma vez. Cliente que respondeu fica aguardando
      -- resposta humana: nenhum follow-up automatico ate outbound novo.
