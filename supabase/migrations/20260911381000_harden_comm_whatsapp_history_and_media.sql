...
VALUES ('comm-whatsapp-media', 'comm-whatsapp-media', false)
ON CONFLICT (id) DO UPDATE
SET public = false;

CREATE INDEX IF NOT EXISTS idx_comm_whatsapp_messages_chat_latest_preview
  ON public.comm_whatsapp_messages (chat_id, message_at DESC, created_at DESC, id DESC);
...
AS $$
  ...
  SELECT
    ...
    c.manual_unread,
    c.manual_unread_at,
    COALESCE(NULLIF(btrim(c.last_message_text), ''), NULLIF(btrim(latest_message.preview_text), '')) AS last_message_text,
    COALESCE(NULLIF(btrim(c.last_message_direction), ''), latest_message.direction) AS last_message_direction,
  ...
  WHERE public.current_user_can_view_comm_whatsapp()
    ...
    AND (
      ...
      OR COALESCE(latest_message.preview_text, '') ILIKE '%' || input.search_text || '%'
