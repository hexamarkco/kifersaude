...
-- A listagem do Inbox precisa preservar a identidade salva, mas não pode
-- executar a função de normalização de telefones contra todo o cache para
-- cada chat. O fallback abaixo usa a chave exata e o índice
-- (channel_id, phone_digits); variantes de telefone continuam sendo
-- resolvidas pelo refresh da identidade e pelo lookup assíncrono do frontend.
