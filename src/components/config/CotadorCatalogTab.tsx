...
export default function CotadorCatalogTab({ embedded = false }: CotadorCatalogTabProps) {
  ...
  const handleEntitySubmit = async (event: FormEvent) => {
    ...
    if (result.error) {
      showMessage('error', 'Erro ao salvar item do catalogo.');
      setSubmitting(false);
      return;
    }
  ...
  const handleProductSubmit = async (event: FormEvent) => {
    ...
    if (!operadoraId) {
      showMessage('error', 'Nao foi possivel resolver a operadora desta linha.');
      return;
    }
  ...
  const handleDelete = async (
    ...
    if (result.error) {
      showMessage('error', 'Erro ao excluir item do catalogo.');
      return;
    }
    ...
    <div className={containerClass}>
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm text-[color:var(--panel-text-soft,#5b4635)]">
              Exemplo real: operadora Amil, linhas diferentes, produtos como Bronze ou S750 e tabelas separadas por MEI, coparticipacao e faixa de vidas.
            </p>
          </div>
      ...
      {loading ? (
        ...
        administradoras.length === 0 ? (
          <EmptyState icon={ShieldCheck} title="Nenhuma administradora cadastrada" description="Cadastre administradoras para suportar adesao e produtos com elegibilidade institucional." />
        ) : (
        ...
        entidades.length === 0 ? (
          <EmptyState icon={Sparkles} title="Nenhuma entidade cadastrada" description="Cadastre entidades, sindicatos e associacoes para relacionar elegibilidade por produto." />
        ) : (
          <div className="grid gap-4 xl:grid-cols-2">
            {entidades.map((item) => (
              <article key={item.id} className="rounded-3xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
              ...
              <article key={line.id} className="rounded-3xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-[color:var(--panel-text-soft,#5b4635)]">Operadora: {line.operadora?.nome ?? 'Nao encontrada'}</p>
              ...
              <article key={product.id} className="rounded-3xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-sm text-[color:var(--panel-text-soft,#5b4635)]">{product.operadora?.nome ?? 'Operadora'} / {product.linha?.nome ?? 'Linha nao definida'}</p>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button variant="icon" size="icon" className="h-10 w-10 text-red-600 hover:bg-red-50" onClick={() => void handleDelete('produtos', product.id, 'Excluir produto', 'Essa ação remove o produto do catálogo do Cotador, mas snapshots de cotacoes permanecem preservados.') }><Trash2 className="h-4 w-4" /></Button>
                  </div>
                </div>
              </article>
            ))}
            ...
            <article key={table.id} className="rounded-3xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface,#fffdfa)] p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="icon" size="icon" className="h-10 w-10 text-red-600 hover:bg-red-50" onClick={() => void handleDelete('tabelas', table.id, 'Excluir tabela', 'Essa ação remove a tabela comercial, mas snapshots já usados em cotacoes permanecem preservados.') }><Trash2 className="h-4 w-4" /></Button>
                </div>
              </div>
        ...
        <form onSubmit={handleEntitySubmit} className="space-y-5">
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-[color:var(--panel-border-subtle,#e7dac8)] bg-[var(--panel-surface-soft,#f4ede3)] p-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-[color:var(--panel-text-soft,#5b4635)]">Mantem o registro disponível para uso no catálogo.</p>
            </div>
          </label>
          <div>
        ...
        <form onSubmit={handleProductSubmit} className="space-y-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <FilterSingleSelect icon={MapPin} options={abrangenciaOptions} placeholder="Selecione a abrangencia" value={productForm.abrangencia} onChange={(value) => setProductForm((current) => ({ ...current, abrangencia: value }))} />
            </div>
            <div>
              <label className="mb-2 block text-sm font-medium text-[color:var(--panel-text-soft,#5b4635)]">Acomodacao</label>
              <FilterSingleSelect icon={Sparkles} options={acomodacaoOptions} placeholder="Selecione a acomodacao" value={productForm.acomodacao} onChange={(value) => setProductForm((current) => ({ ...current, acomodacao: value }))} />
            </div>
            <div>
        ...
        <form onSubmit={handleTableSubmit} className="space-y-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <Input value={tableForm.codigo} onChange={(event) => setTableForm((current) => ({ ...current, codigo: event.target.value }))} placeholder="Ex: S750-R1-MEI-TOTAL-2A2" />
            </div>
            <div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-[color:var(--panel-text-soft,#5b4635)]">Perfil empresarial</label>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-[color:var(--panel-text-soft,#5b4635)]">Coparticipacao</label>
              <FilterSingleSelect icon={Sparkles} options={[...coparticipacaoOptions]} placeholder="Selecione a coparticipacao" value={tableForm.coparticipacao} onChange={(value) => setTableForm((current) => ({ ...current, coparticipacao: value as TableFormState['coparticipacao'] }))} />
            </div>
            <div>
