...
BEGIN
  ...
  ELSIF v_lead.telefone IS NOT NULL AND v_lead.telefone <> '' THEN
    SELECT count(*), (ARRAY(SELECT c2.id FROM public.comm_whatsapp_chats c2 WHERE c2.deleted_at IS NULL AND public.comm_whatsapp_phone_lookup_keys(c2.phone_digits) && public.comm_whatsapp_phone_lookup_keys(v_lead.telefone) LIMIT 1))[1]
    INTO v_match_count, v_matched_chat_id
    FROM public.comm_whatsapp_chats c
    WHERE c.deleted_at IS NULL
      AND public.comm_whatsapp_phone_lookup_keys(c.phone_digits)
          && public.comm_whatsapp_phone_lookup_keys(v_lead.telefone);

    IF v_match_count = 1 AND v_matched_chat_id IS NOT NULL THEN
...
console.log('Fixing audit_classify_single_lead (min(uuid) bug)...');
