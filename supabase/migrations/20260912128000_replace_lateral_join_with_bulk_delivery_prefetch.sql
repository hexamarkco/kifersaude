-- Replace the per-row LATERAL JOIN fetching delivery_status with a single
-- bulk DISTINCT ON prefetch. Under write load (webhooks persisting messages),
-- 80-250 individual index probes on comm_whatsapp_messages accumulate latency
-- and cause statement_timeout. A single index scan for the entire page avoids
