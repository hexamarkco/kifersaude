-- Agora que comm_whatsapp_chats.last_message_delivery_status e uma coluna
-- real, mantida pelo trigger adicionado em
-- 20260913130000_add_comm_whatsapp_chat_last_message_status_sync.sql,
-- comm_whatsapp_list_chats nao precisa mais recalcular esse valor a cada
-- chamada com um DISTINCT ON sobre comm_whatsapp_messages. Le-se a coluna
-- direto do chat, eliminando essa subconsulta extra do caminho mais
-- executado do inbox (polling da lista a cada 8-60s, mais toda acao que
-- recarrega a lista).
...
AS $$
  ...
  SELECT
    ...
    c.last_message_at,
    c.last_message_delivery_status,
    c.unread_count,
    c.status,
    c.last_read_at,
    c.created_at,
